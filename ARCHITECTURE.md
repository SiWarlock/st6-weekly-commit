...
`apps/wc-web` — React 18, Vite 5, TS strict, Flowbite React + Tailwind utilities, **RTK Query for all API calls** (no Saga/Thunk, no CSS Modules/styled-components, no SSR). Module Federation remote exposing **one** module (`./WeeklyCommitApp`); shared singletons: React, React DOM, Redux Toolkit, React-Redux, **React Router DOM** (**5**, realized Deploy 2 — the remote **self-provides its store** + injects its own CSS; same-origin `/portal/` host, OQ-004).
...
Two synchronous projections (ADR-006): `manager_plan_summary` (one row per manager/report/week — plan_state, review_status, `is_review_overdue`, planned/unplanned/misaligned/needs-review/blocked/carry-forward/unresolved-dispute counts) and `manager_heatmap_cell` (manager × report × week × Defining Objective — counts + enumerated `risk_badges`). Rows update **synchronously in the same transaction** as commitment create/update/delete, plan lock, reconciliation start/close, dispute open/respond/resolve, mark-reviewed, and carry-forward. **Count derivation pinned:** `misaligned_count` = commitments with `alignment_status=MISALIGNED` **OR** an open (`OPEN`/`IC_RESPONDED`) dispute with `flag_type=MISALIGNED` (deduplicated per commitment); `unresolved_dispute_count` = commitments with an `OPEN`/`IC_RESPONDED` dispute; `blocked_count` = commitments with `reconciliation_outcome=BLOCKED` (the reconciliation **risk-outcome** — sibling of `carry_forward_count` = `reconciliation_outcome=CARRIED_FORWARD`; **not** `work_type=BLOCKER`, which is a planning *category*, so `blocked_count=0` at lock before any outcome is recorded — pinned by the R5 Grace seed + `rebuild==seed`, §17); `REVIEWED_WITH_DISPUTES` keeps `is_review_overdue=false` while `unresolved_dispute_count` keeps surfacing risk (REQ-F-013). An internal rebuild **job/CLI** recomputes both tables from source (no admin UI). The recompute is **recompute-then-prune, not upsert-only**: it DELETEs any `manager_heatmap_cell` whose Defining Objective the new source state no longer produces (e.g. a dispute-respond rule-#2 SO revision remaps a commitment's DO) — so incremental == rebuild with no orphan cells (the summary grain is stable; only cells go stale) (§17/RISK-003). **Command-center filters (REQ-F-023)** — person, plan-state, review-state served from summary columns; Defining Objective from the heatmap grain; priority/work-type/alignment-status via the indexed commitment-level columns added in §4.
...
**CronJob = `wc-api` image** in generation mode (no third ECR image); reads the DB secret only. **Secrets injection:** Secrets Store CSI Driver + ASCP; Spring reads the mount as a **config tree** (`spring.config.import=optional:configtree:/mnt/secrets/`) — each secret key is mounted as a file named by the property key (`spring.datasource.url`, …`/jwt.issuer-uri`, `auth0.audience`, `GRAPH_*`), so ONE `optional:` import binds db+auth0+graph (realized Wave-1 datasource slice). **`configtree:`, NOT `file:`** — a bare-value-per-file mount needs config-tree binding; `file:` expects a single parseable config document. The deploy profile is `aws` (config in `application-aws.yml`; an `application-prod.yml` is dead under an `aws` profile). REQ-S-004's "environment variables" is intent, a file mount satisfies it. **Per-workload IRSA least privilege:** api SA → `sns:Publish` + `secretsmanager:GetSecretValue` (db/auth0/graph); worker SA → `sqs:{ReceiveMessage,DeleteMessage,GetQueueAttributes}` on queue+DLQ + `GetSecretValue` (db + graph — db added Wave-2 s8 `adfa639` for the worker's SyncRecord reload; still least-privilege, NOT auth0/demo/`*`); CronJob SA → `GetSecretValue` (db only); migration Job SA → `GetSecretValue` (db only). **CORS** (Spring-owned; ALB/CloudFront pass through): allow origin exactly `https://wc.${ROOT_DOMAIN}` (+ `http://localhost:<vite-port>` dev), methods `GET/POST/PATCH/DELETE/OPTIONS`, headers `Authorization, Content-Type, X-Demo-Employee-Id`, `allowCredentials=false` (bearer transport), preflight `OPTIONS` bypasses JWT; demo mode must **not** relax CORS.
...
Carried from `OPEN_QUESTIONS.md`; none block finalization. **OQ-001** exact `ROOT_DOMAIN` (Terraform variable until supplied). **OQ-002** AWS region override (default `us-east-1`). **OQ-003** exact Auth0 claim-name defaults (configurable mapper + demo defaults). **OQ-004 ✅ REALIZED (Deploy 2, 2026-06-06)** — the "Acme Portal" MF host embeds the remote **same-origin** (`wc.${ROOT_DOMAIN}/portal/`, Option B — an additive CloudFront `/portal/*` behavior + a viewer-request rewrite Function, no cross-origin CORS). Realized contract: **1 expose** (`./WeeklyCommitApp`), the remote **self-provides its store** + **injects its own CSS**, **5 shared singletons** (react, react-dom, react-redux, @reduxjs/toolkit, react-router-dom), and the host passes only the auth accessor (registered **synchronously** in the exposed module's render). See sessions 024/025, wc-web LESSONS §31–§33, infra LESSONS §23. (A real cross-origin `portal.` distribution is deferred to AWS account CloudFront-verification — Carry-forward.) **OQ-005** real M365 tenant/app registration + whether `Calendars.ReadWrite` application access needs an **application access policy / mailbox scoping** in the target tenant (hybrid demo adapter otherwise). **OQ-006** evaluator-specific demo-video / AI-usage templates (generic Markdown otherwise). **OQ-007 (new)** the PRD literal "PostgreSQL 16.4" is superseded by "latest 16.x minor" (16.4 is no longer creatable on RDS) — sign-off recorded §"Locked decisions". **OQ-008 (new)** Spring Boot 3.3 is OSS-EOL (2025-06-30); pin kept for the assessment with EOL documented.
...
infra/k8s/
  ...
  serviceaccount-worker.yaml       # IRSA: sqs Receive/Delete/GetAttrs on queue+DLQ + GetSecretValue (db + graph)
...
| `GRAPH_TENANT_ID` / `GRAPH_CLIENT_ID` / `GRAPH_CLIENT_SECRET` | Graph adapter (real) | *(secret)* | Secrets Manager via CSI | O | Required only when `GRAPH_MODE=real`. Worker SA reads **db + graph secrets** (least privilege, §12 — db added Wave-2 s8 `adfa639` for the SyncRecord reload; the real Graph adapter realized Wave-2 s9 `becc22a`). `Calendars.ReadWrite` application permission; mailbox-scoping per OQ-005. |
...
**Delivery (§12/§13).** Seed ships as ordered Flyway migrations owned solely by the pre-deploy migration Job, **after** the schema/index/projection DDL (V1–V3): `V4__seed_rcdo.sql` (read-only RCDO reference, kept in `classpath:db/migration` so the integration-test harness still serves it), and `V5__seed_personas_and_relationships.sql` + `V6__seed_fixture_plans_and_state.sql` (the demo persona/fixture seed, in a **separate `classpath:db/demo-seed` location** — the migrate Job loads both via `locations: classpath:db/migration,classpath:db/demo-seed`, while the shared `SharedPostgres` integration-test harness, on `db/migration` only, never seeds demo data; realized V5/Opt-1). All seed rows use **fixed UUIDs** (literal `uuid` constants, e.g. `'a0000000-0000-0000-0000-000000000001'`) and **fixed timestamps anchored to demo anchor date 2026-06-02** (Tuesday; current week `2026-06-01`→`2026-06-07`, prior week `2026-05-25`→`2026-05-31`, org tz `America/Chicago`). Idempotency: every `INSERT … ON CONFLICT (<unique key>) DO NOTHING`. Re-running migration on an existing DB is a no-op (deterministic across runs). The 2,000-record synthetic perf seed is **not** here — see end of section.
...
`employee.email` unique; all `timezone='America/Chicago'`; `active=true`. **`employee.email` is the live-Outlook mailbox target** — the worker's Graph adapter writes each persona's calendar event to their `employee.email`, so the seed sets it to the deploy's **M365 tenant domain** (`@dreddy817.onmicrosoft.com`, the user's E3 tenant — real licensed mailboxes; **deploy-data, re-seeded post-098** at `87daf80`). This is **distinct from the Auth0 login email** (kept `@st6demo.com`, cosmetic): login resolves by the `external_subject` claim, **not** email (there is no `findByEmail`), so the domain mismatch is inert for auth. `external_subject` is **POPULATED** with a deterministic app-owned literal `st6|<first>-<last>` (e.g. `st6|dana-okafor`, distinct per persona) — the deployed demo uses **real Auth0/OAuth2**, so a validated JWT's `employee_id` claim resolves to the seeded employee via `PrincipalResolver.resolve(Auth0Identity)`→`findByExternalSubject` (realized V5, brief 084). Demo-auth mode (`X-Demo-Employee-Id` = the employee `id`) remains the dev/local identity path (§6 / F.1).

| Persona | display_name | email | role |
|---|---|---|---|
| **M** (manager) | Dana Okafor | dana.okafor@dreddy817.onmicrosoft.com | MANAGER |
| R1 | Priya Raman | priya.raman@dreddy817.onmicrosoft.com | IC |
| R2 | Marco Bellini | marco.bellini@dreddy817.onmicrosoft.com | IC |
| R3 | Aisha Khan | aisha.khan@dreddy817.onmicrosoft.com | IC |
| R4 | Tomas Novak | tomas.novak@dreddy817.onmicrosoft.com | IC |
| R5 | Grace Liu | grace.liu@dreddy817.onmicrosoft.com | IC |
| R6 | Sam Carter | sam.carter@dreddy817.onmicrosoft.com | IC |
...
**Heatmap / command-center coverage (REQ-E-002/E-003).** Across the six reports, current-week `manager_heatmap_cell` rows for Dana span all three Defining Objectives with varied `risk_badges` drawn only from the enumerated vocabulary: `MISALIGNED` (R3), `OVERDUE_REVIEW` + `UNREVIEWED` (R2), `BLOCKED` (R5), `CARRY_FORWARD` (R5), `NEEDS_REVIEW` (R3). `manager_plan_summary` rows mirror counts. These projection rows are **NOT hand-seeded** — they populate on deploy via the `ProjectionRebuildRunner` (`--app.job=rebuild-projections`, run after the migration Job — truncate + recompute-from-source, §9/§17/wc-api LESSONS §40). The V6 source fixtures ARE the truth; the rebuild derives the projections from them (so `rebuild==seed` is moot — the rebuild IS the population). Realized: V6 (source, 10.3–10.5) + infra task 12.14 (the rebuild Job + deploy.yml step). **Negative-auth fixture:** because no `manager_relationship` makes any report manage another, any cross-report manager access in the demo (e.g. R1 attempting Dana's command center) is denied `404`/`403` (REQ-E-002 "denied manager view").
