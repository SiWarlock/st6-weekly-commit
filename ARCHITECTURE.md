...
- **Manager scope:** direct reports only; drafts viewable, but formal review/dispute begins only after lock.
...
| `hasUnresolvedDispute` | boolean | no | derived (`OPEN`/`IC_RESPONDED` exists) |
...
**E19 request — `ResolveDisputeRequest`** (POST `/api/disputes/{id}/resolve`; status `OPEN`/`IC_RESPONDED`, direct manager): no required body (optional `resolutionNote` string). IC attempt → `403 IC_CANNOT_RESOLVE_DISPUTE`. Resolving the last unresolved dispute re-evaluates parent `REVIEWED_WITH_DISPUTES → REVIEWED` (§3).
...
Shape: `{ type, title, status, detail, safeMessage, code, constraint?, fieldErrors?[], traceId }`. Named `code`s: `EMPTY_PLAN_LOCK`, `UNLINKED_PLANNED_COMMITMENT`, `LOCKED_BASELINE_EDIT`, `SECOND_OPEN_DISPUTE`, `IC_CANNOT_RESOLVE_DISPUTE`, `UNPLANNED_MISSING_LINK_AT_CLOSE`, `ILLEGAL_STATE_TRANSITION`. Status map: `400/422` validation, `403` authorization denial, `404` not-found-or-not-authorized (IDOR-safe), `409` state-precondition/conflict + optimistic-lock.
...
| `VITE_API_BASE_URL` | RTK Query base query | `http://localhost:8080` (dev) · `https://api.wc.${ROOT_DOMAIN}` (deployed) | Vite build env | R | Host-overridable in remote mode (PA host may inject base URL). |
