...
## <a id="23"></a>23. Deploy-2 same-origin PATH federation (Option B) — an additive `/portal/*` behavior + a viewer-request rewrite Function for per-path SPA fallback; the fail-safe-to-remote main-site build trap; prod writes stay user-gated

**Date:** 2026-06-06.
**Source slice:** 025 (Deploy 2 — Module-Federation portal; the cross-origin → same-origin pivot).

Deploy 2 (a federation host — "Acme Portal" — embedding the Weekly Commit micro-frontend) was first authored **cross-origin** (a separate `portal.st6weeklycommit.com` distribution + cert + bucket) and **failed at `terraform apply`**: `403 AccessDenied — "account must be verified before you can add new CloudFront resources"` — an AWS **account-level** gate, not our code. Pivoted to **Option B: serve the host from a PATH on the existing `wc.` distribution** → no new distribution, no new cert, no account gate, and **same-origin moots cross-origin CORS** entirely. Three durable facts:

- **Same-origin path federation = an ADDITIVE cache behavior + a viewer-request rewrite Function.** Add a `/portal/*` cache behavior on the **existing** distribution (same `s3-assets` origin; default + `/remote/*` behaviors left **byte-identical**), host built `--base=/portal/`, remote at `/remote/`. **Per-path SPA fallback needs a CloudFront Function, not custom-error-responses** — distribution-wide custom errors return the **main** `/index.html`, so `/portal/callback` would load the wrong SPA and break Auth0. A viewer-request rewrite Function (extension-less `/portal/*` → `/portal/index.html`) is the ONLY way to do per-path fallback. (Both the lead and frontend independently flagged this as the #1 gotcha — build it proactively.)
- **The fail-safe-to-remote main-site build trap (composes wc-web LESSON 28).** wc-web's `vite.config` is **fail-safe-to-remote** — a build with no `VITE_BUILD_TARGET` emits the **remote**, not the standalone SPA. So "just remove the standalone var" from the main-site build step would ship the **remote** to the site root and break the live site. The safe move is to **SKIP the main-site build step entirely** during a review window (leaving the live root byte-identical to the recorded core), NOT to drop the env var. Audit any deploy step that builds wc-web for an explicit `VITE_BUILD_TARGET=standalone` — its absence silently degrades to the remote.
- **Prod writes stay user-gated — the republish loop runs as user-executed `s3 sync`.** The auto-mode classifier blocks direct `aws s3 sync --delete` + `create-invalidation` to the prod bucket (a production write bypassing the user-push/approval gate). Honor it: the post-deploy MF-fix republish loop runs as **user-executed** `s3 sync` (Option A); decline a standing allow-rule (Option B) to preserve the project's USER-controlled-prod-write posture. Document the exact `s3 sync … /remote/ --delete` + `cloudfront create-invalidation --paths "/remote/*"` recipe for the user to run.

Verified via the validate/policy path (infra is not red-green TDD): `terraform fmt -check`/`validate`/`tflint` clean, `actionlint` on `deploy.yml`, `kubeconform -strict` on the manifests, + post-deploy curl plumbing (`/portal/` → host index 200, `/remote/assets/remoteEntry.js` → 200, `/portal/callback` → host index via the rewrite, root `/` unchanged). The dropped cross-origin `portal.tf` partials (cert ISSUED, empty bucket) are left dormant (~$0) for a fast clean re-run once the AWS account is CloudFront-verified.

**Rule:** when a new CloudFront distribution is blocked (account-verification gate) or cross-origin CORS is unwanted, serve a federation host **same-origin via an additive `/portal/*` cache behavior on the existing distribution** + a **viewer-request rewrite Function** for per-path SPA fallback (distribution-wide custom-error responses can't do per-path). Never "remove the standalone build var" to repoint the main site — wc-web is fail-safe-to-remote (wc-web LESSON 28), so SKIP the build step instead; audit every wc-web build step for an explicit `VITE_BUILD_TARGET=standalone`. Keep prod object writes user-executed (the classifier gate is correct — don't add a standing allow-rule). (origin: 025 / Deploy 2)
