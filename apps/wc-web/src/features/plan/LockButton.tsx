...
/**
 * Lock affordance (rule #1, E8). Enabled **only** when `LOCK ∈ plan.allowedActions`
 * — the frontend never re-derives lock eligibility (an unlinked/empty plan is
 * rejected by the SERVER). On click it invokes E8 (invalidate→refetch into
 * LOCKED, no optimistic flip). A blocked lock renders the server `safeMessage` +
 * `fieldErrors[]` verbatim (`409 UNLINKED_PLANNED_COMMITMENT`/`EMPTY_PLAN_LOCK`).
 */
export function LockButton({ plan }: { plan: WeeklyPlanDto }) {
  ...
  const enabled = can('LOCK', plan.allowedActions);
