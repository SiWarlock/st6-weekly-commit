 ...
 * infra-owned {@code job-seed-demo.yaml} Job) runs {@link DemoSeeder#run(LocalDate)} once for the
 * target week.
 *
 ...
 * regardless of the JVM zone). {@link DemoSeeder#run(LocalDate)} re-normalizes defensively, so
 ...
 * the logic lives in {@link DemoSeeder}; this runner's gating + arg-parsing + delegation are proven
 * via {@code ApplicationContextRunner}. Mirrors {@code ProjectionRebuildRunner} / {@code
...
public class DemoSeedRunner implements ApplicationRunner {
  ...
  public DemoSeedRunner(DemoSeeder seeder, OrgTimeConfig orgTimeConfig, Clock clock) {
    this.seeder = seeder;
  ...
  public void run(ApplicationArguments args) {
    ...
    seeder.run(weekStart);
    log.info("Demo-seed job complete for week {}; one-shot exit.", weekStart);
  }
}
