...
| `ErrorCodes` (named-code constants, `api/web/`) | §5 (RFC-7807 named codes) | The §5 RFC-7807 `code` vocabulary as `static final String` constants — **NOT an `enums/` member** (free-string codes, no `VARCHAR`+`CHECK`; like `AllowedAction`/the §15 audit-action taxonomy). Started at 3.4a with `VALIDATION_ERROR` + `ILLEGAL_STATE_TRANSITION`; accretes per slice (`LOCKED_BASELINE_EDIT` at 3.4b; `EMPTY_PLAN_LOCK`/`UNLINKED_PLANNED_COMMITMENT` at 3.5). Each new code must match a §5 named code; a new code pairs a §5 edit if §5 doesn't already list it. (origin: 3.4a) |
