...
**Addendum (6.5b) — manager-surface denial endpoints write audits → their endpoint tests need audit-teardown.** The same teardown class bites the read surfaces too: a manager **denial** endpoint (E14 IC-denied `403`, E15 own-cell `404`) writes an `audit_event` via the central `recordDenial` (`REQUIRES_NEW`, survives the rolled-back read). An endpoint test that exercises a denial leaves an `audit_event` row — its `@AfterEach` must `auditEvents.deleteAll()` (FK-free here, but the rows accumulate and can perturb a **count-based** assertion in a sibling denial test). 6.5b's `heatmap_icDenied403` needed the audit-teardown added mid-GREEN. **Forward note for 6.6 (the IDOR matrix):** every denial-case endpoint test writes a denial audit — clear `audit_event` in teardown across the *whole* matrix, and prefer asserting the specific `AUTHORIZATION_DENIED` row (action + resource) over a bare `count() > 0` so accumulation can't make a stale assertion pass. (origin: 6.5b)
...
## <a id="40"></a>40. A from-source rebuild job = truncate + recompute-all via the SAME per-unit entrypoint as the incremental path (drift correction for free); prove it with corrupt-then-rebuild, not build-then-rebuild

**Date:** 2026-06-04.
**Source slice:** 6.7 (the ProjectionRebuildRunner — REQ-D-013, the last Phase-6 slice).

A read-model **rebuild** job should reuse the **same per-unit derivation entrypoint the incremental path uses** — here `ProjectionRefresher.recomputeForPlan` (the §8/§23-extracted source-loading entrypoint the 9 synchronous triggers already call, 6.3a). The rebuild = **truncate** the read-model tables + iterate the source units calling that one entrypoint. Then **`rebuild==incremental` holds by construction** (one derivation, not two that can diverge), and **drift/orphan correction comes for free** (the truncate clears stale rows; the recompute re-derives from source — the recompute-then-prune of §37 is moot on a clean slate). One `@Transactional` run = an **atomic swap** (no reader sees a half-rebuilt projection). The §23 one-shot pattern carries the rest: a thin `@ConditionalOnProperty(app.job=…)` `ApplicationRunner` + a plain logic `@Component`, `--spring.main.web-application-type=none` exit, one SYSTEM null-actor audit row per run (a free-string job action like `PROJECTIONS_REBUILT`, §15).

**The test trap:** a `build → rebuild → assert-equal` test is **near-tautological** — both sides call the same entrypoint, so it proves nothing about the rebuild's actual job. Give it teeth with **corrupt-then-rebuild-then-assert-corrected**: build the correct projection → **mutate a count / delete a cell / inject an orphan cell** (a grain the source no longer produces) → run the rebuild → assert it is back to correct-from-source with the orphan pruned. That pins the rebuild's real value (reconstruction / drift correction), which the tautological form misses.

**JPA note:** `deleteAllInBatch()` (the bulk truncate) bypasses the L1 persistence context, but a rebuild that loads **no** read-model entity *before* the truncate has no staleness to flush — so **drop the defensive `em.flush()/clear()`** (and don't add a `@PersistenceContext` field: it makes the `PersistenceAnnotationBeanPostProcessor` try to inject into the mock in a no-JPA `ApplicationContextRunner` gating test → `BeanCreationException`, breaking the mockable seam). Composes §23 (one-shot job) + §37 (recompute-then-prune). (origin: 6.7)

## <a id="41"></a>41. A demo/fixture Flyway seed lives in a SEPARATE `db/demo-seed` location the deploy Job loads (two-location) but the shared test harness never does; read-only reference seed stays in `db/migration`

**Date:** 2026-06-04.
**Source slice:** V5 (brief 084 — the first deployed-demo backend slice; OAuth `external_subject` seed).

The shared `SharedPostgres` integration-test harness runs Flyway on `classpath:db/migration` and asserts a **clean baseline** (e.g. `employees.count()==0`). Putting demo personas/fixtures in `db/migration` would **pollute every integration test** (the harness would see 7 seeded employees → broken count assertions). But the deployed demo genuinely needs that seed.

- **Split by Flyway location (Opt-1).** The demo persona/fixture seed (V5 personas, V6 fixture state) lives in a dedicated **`classpath:db/demo-seed`** location. The **deploy migrate profile loads BOTH**: `spring.flyway.locations: classpath:db/migration,classpath:db/demo-seed` (`application-flyway-migrate.yml`) — so the pre-deploy migration Job seeds the demo. The shared test harness loads **`db/migration` only** — so demo rows never enter it.
- **Read-only REFERENCE seed stays in `db/migration`.** V4 RCDO is the link-target reference data tests legitimately depend on (every locked planned commitment references an SO) — keep it where tests see it. Only the *demo persona/fixture* seed (which would pollute count baselines) moves out.
- **Flyway orders by VERSION across all locations**, not by location — so `V5`/`V6` in `db/demo-seed` still run after `V4` in `db/migration`. The version number sets order; the location only sets *who loads it*.
- **The migrate-profile `locations` line is production config → pin it** (forbidden-pattern #1): extend `FlywayProfilePropertyTest` to assert `flyway-migrate` resolves `spring.flyway.locations` to BOTH locations, base/local/demo to null. This pins the load-bearing "deploy seeds, tests stay clean" property — a deleted `locations` line would silently ship a demo with no personas.
- **Verify the seed in an ISOLATED per-class Testcontainer** loading both locations (`V5SeedPersonasMigrationTest` migrates `db/migration`+`db/demo-seed` in its own ephemeral PG) — proves the seed applies + is idempotent + resolves, **without** touching the shared `SharedPostgres` harness (which stays clean).
- **Gotcha:** a root `./gradlew test --tests <filter>` fails `:worker:test` with "No tests found for given includes" — when running a filtered subset, scope per-module (`:shared:test --tests … :api:test --tests …`).

**Rule:** demo/fixture seed → a dedicated `db/demo-seed` Flyway location loaded only by the deploy migrate Job (two-location `locations`), never by the shared integration-test harness; read-only reference seed stays in `db/migration`; pin the migrate-profile `locations` as production config; verify the seed in an isolated container, not the shared harness. (origin: V5/10.2)

**Addendum (V6) — seed the SOURCE rows that match what the lifecycle services PRODUCE; never hand-seed a DERIVED value.** A fixture seed must store the same source the services would write, so the read path reproduces the intended demo state. Two traps surfaced at V6: **(a)** `REVIEWED_WITH_DISPUTES` is a **service-STORED** review status (computed by `ReviewStatusDeriver` at mark-reviewed / dispute-open / resolve and persisted — `ReviewService` / `DisputeService`; `ReviewMapper` returns `getStatus()` directly), **NOT** read-derived — so the seed stores it (a brief that said "store `REVIEWED`, derive `WITH_DISPUTES` at read" was inverted; the impl caught it against the code + the MVP 10.3 spec). Only `OVERDUE` is genuinely read-derived (`NOT_REVIEWED` + past `review_due_at` under the injectable `Clock`, never stored — rule #6). **(b)** The §9 **projections** (`manager_plan_summary` / `manager_heatmap_cell`) are NOT hand-seeded either — seed only the source, and let the `ProjectionRebuildRunner` (the deploy-time rebuild Job, §40 / task 12.14) derive them. **Rule:** distinguish service-STORED facets (seed them, matching the service's own write) from read-DERIVED facets (seed the inputs, let the read derive) — a hand-seeded derived value silently disagrees with the live read path. (origin: V6/10.3)

## <a id="42"></a>42. Deploy-secret binding via `configtree:/mnt/secrets/` (one import binds db+auth0+graph); an `application-<profile>.yml` is DEAD under a different active profile

**Date:** 2026-06-04.
**Source slice:** 090 (deploy datasource + the aws-profile fix — Wave-1 s3).

The deployed app reads secrets from a CSI tmpfs mount where the SecretProviderClass extracts each secret key into a file **named by the property key** (`spring.datasource.url`, `spring.security.oauth2.resourceserver.jwt.issuer-uri`, `auth0.audience`, `GRAPH_*`). Two load-bearing facts:

- **Bind with `spring.config.import=optional:configtree:/mnt/secrets/`, NOT `file:`.** Config-tree treats each file as one property (filename == property key), so ONE import binds the datasource (`spring.datasource.*` → Hikari auto-configures) + auth0 issuer-uri/audience + graph — whatever that workload's SPC mounts. `file:` expects a single parseable config document; a bare-value-per-file mount needs `configtree:`. `optional:` keeps local/demo (no mount) booting. Config-tree has HIGHER precedence than the importing yaml, so it overrides base `${ENV:}` defaults — keep a literal (`auth0.audience: https://api.wc.${ROOT_DOMAIN}`) as a sane pre-secret-population default the mount overrides.
- **A profile-named yaml only loads under THAT active profile.** Every k8s workload ran `SPRING_PROFILES_ACTIVE=aws`, but the deploy config lived in `application-prod.yml` (profile `prod`) → Spring looked for `application-aws.yml` (absent) and **`application-prod.yml` never loaded** → the deployed api booted with a blank `auth0.audience` + no datasource. **Verify the config filename matches the active profile** (`application-<activeProfile>.yml`); a `prod`-named file is silently dead under an `aws` profile (no `spring.profiles.group` bridge existed — confirmed). `ddl-auto=validate` (migration Job owns the schema; fail-fast on entity↔DDL drift) gates db-connectivity at boot, so readiness needn't include db.

**Rule:** deploy-secret config = `spring.config.import=optional:configtree:/mnt/secrets/` (filename==property-key; `optional:` for local; literals as overridable defaults), NOT `file:`; and the config file MUST be named for the ACTIVE profile — a mismatched profile name silently doesn't load. (origin: 090)
