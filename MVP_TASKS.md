...
**Bootstrap session.** Architecture finalized; `MVP_TASKS.md` generated; scaffolding not yet generated; first `/tdd` slice not yet started.

**Next session target:** `0.1` (monorepo root) — but run `/scaffold-generate` first to personalize the agent-team harness.
...
### 0.1 — Monorepo root: Yarn Workspaces + Nx + Gradle root wiring
- [ ] Root `package.json` declares Yarn Workspaces over `apps/*` (`wc-web`, `wc-e2e`) and pins Node/Yarn engines; `nx.json` configures Nx project graph + cache (assessment-light, ASM-001) and does NOT add PA-shell ownership (no global routing/LogRocket/Loki/shell nav config), satisfying REQ-I-008.
- [ ] Top-level dir skeleton created exactly per Appendix C.1: `apps/{wc-web,wc-api,wc-e2e}/`, `infra/{terraform,k8s}/` (empty placeholders with `.gitkeep`), `docs/` already present; root holds `nx.json`, `package.json`, `settings.gradle`, `build.gradle`, `gradle.properties`, `docker-compose.yml`.
- [ ] `yarn install` succeeds and `npx nx graph --file=/dev/null` (or `nx show projects`) lists the JS projects with no errors; no domain code.
- [ ] Files: NEW `package.json` (root), NEW `nx.json` (root), NEW `apps/.gitkeep`, NEW `infra/terraform/.gitkeep`, NEW `infra/k8s/.gitkeep` (per Appendix C.1)
- [ ] Cross-doc invariant: none (no typed model)
- [ ] Tests — happy: `yarn install` clean + `nx show projects` lists `wc-web` and `wc-e2e`; edge: re-run of install is idempotent (no lockfile drift); error: workspace glob pointing at a missing package fails fast in CI; integration: Nx graph resolves with zero unresolved project refs
- [ ] Requirements: ASM-001, REQ-I-008

### 0.2 — Gradle multi-module build + Java 21 toolchain + quality gates enforced
- [ ] `apps/wc-api/settings.gradle` sets `rootProject.name='wc-api'` and `include 'shared','api','worker'`; `apps/wc-api/build.gradle` configures Java 21 toolchain, Lombok with `@Getter @Setter @Builder` only (never `@Data`), and applies Spotless, SpotBugs, JaCoCo to every subproject; `gradle.properties` pins `springBootVersion=3.3.x` and version-catalog pins (Appendix C.2).
- [ ] JaCoCo coverage verification gate is wired **per Gradle module** (api/worker/shared) at ≥80% and fails the build below threshold (§13/§17); Spotless `check` and SpotBugs are bound into `check` so `./gradlew check` runs all three gates.
- [ ] `:shared` `build.gradle` carries JPA/DTO/enum deps and is depended on by `:api` and `:worker`; `:api` and `:worker` declare `implementation project(':shared')`; module dependency direction is shared←api, shared←worker only (no api↔worker edge), proving the boundary (REQ-O-016).
- [ ] A single trivial placeholder test per module (no domain logic) exists so JaCoCo has classes to measure and the ≥80% gate is exercised green on the skeleton.
- [ ] Files: NEW `apps/wc-api/settings.gradle`, NEW `apps/wc-api/build.gradle`, NEW `apps/wc-api/gradle.properties`, NEW `apps/wc-api/shared/build.gradle`, NEW `apps/wc-api/api/build.gradle`, NEW `apps/wc-api/worker/build.gradle` (Appendix C.2/C.3); root NEW `settings.gradle`/`build.gradle`/`gradle.properties` wire the composite/aggregate per C.1
- [ ] Cross-doc invariant: none (build config only; no model fields)
- [ ] Tests — happy: `./gradlew check` green with all three gates active and modules building in shared→api/worker order; edge: a sub-80% module fails `jacocoTestCoverageVerification`; error: introducing a `@Data` usage or a Spotless violation fails `check`; integration: `:api` and `:worker` both resolve `:shared` classes at compile time
- [ ] Requirements: REQ-O-016, ASM-001

### 0.3 — `:shared` module skeleton: package root + enums + base entity classes (no logic)
- [ ] Create `com.st6.wc` package root and the empty-but-typed enum home so later phases bind verbatim: `enums/PlanState`, `RoleType`, `CommitmentKind`, `Priority`, `WorkType`, `Confidence`, `AlignmentStatus`, `ReviewStatus`, `DisputeStatus`, `FlagType`, `ReconciliationOutcome`, `CommentTargetType`, `SyncRelatedType`, `EventKind`, `SyncStatus`, `RiskBadge` — values copied EXACTLY from Appendix A / Appendix B.1 (e.g. `PlanState{DRAFT,LOCKED,RECONCILING,RECONCILED}`, `ReviewStatus{NOT_REVIEWED,REVIEWED_WITH_DISPUTES,REVIEWED}` with NO stored `OVERDUE`, `RiskBadge{MISALIGNED,NEEDS_REVIEW,BLOCKED,CARRY_FORWARD,UNREVIEWED,OVERDUE_REVIEW}`, `CommentTargetType{PLAN,COMMITMENT}` only).
- [ ] Create the `common/` base scaffolding with NO domain behavior: `common/AbstractAuditingEntity.java` (`@MappedSuperclass` created/updated by+at), `common/PersistableUuidEntity.java` (UUID PK + `@Version` base), `config/ClockConfig.java` (injectable `java.time.Clock` bean per §3/§17), `common/OrgTimeConfig.java` (org tz default `America/Chicago` + Mon–Sun week resolver stub).
- [ ] Domain entity classes are NOT created in this phase (deferred to data-model phase); only enums + common base + the `<domain>` package directories exist as compilable placeholders. Lombok annotations used are `@Getter/@Setter/@Builder` only.
- [ ] Files: NEW `apps/wc-api/shared/src/main/java/com/st6/wc/enums/*.java` (16 enums per C.2), NEW `.../common/AbstractAuditingEntity.java`, `.../common/PersistableUuidEntity.java`, `.../config/ClockConfig.java`, `.../common/OrgTimeConfig.java` (Appendix C.2)
- [ ] Cross-doc invariant: NEW (enum value sets that ARE cross-doc invariants per Appendix A: PlanState, RoleType/Employee.role, CommitmentKind, Priority, WorkType, Confidence, AlignmentStatus, ReviewStatus, DisputeStatus, FlagType, ReconciliationOutcome, CommentTargetType, SyncRelatedType, EventKind, SyncStatus, RiskBadge — these `enums/` values must mirror Appendix A exactly per the C.2 cross-doc note)
- [ ] Tests — happy: a Vitest-free JUnit test asserts each enum's exact value set/cardinality matches Appendix A (e.g. `PlanState.values().length==4`, `ReviewStatus` contains no `OVERDUE`); edge: `RiskBadge` contains exactly the 6 enumerated badges; error: an enum value typo (extra/renamed constant) fails the value-set assertion; integration: `ClockConfig` Clock bean injects in a minimal Spring context
- [ ] Requirements: REQ-O-016

### 0.4 — `wc-api` base Spring Boot app: actuator health/readiness, profiles, no domain
- [ ] `:api` produces a runnable Spring Boot 3.3 app `WcApiApplication.java` (`@SpringBootApplication`) that boots with no domain endpoints; exposes `GET /actuator/health/liveness` and `GET /actuator/health/readiness` (used by k8s probes per §15/§12), returning UP on the skeleton.
- [ ] Profile config files exist per Appendix C.2: `application.yml` (base), `application-prod.yml`, `application-local.yml`, `application-demo.yml`, `application-flyway-migrate.yml`; `spring.flyway.enabled=false` in base/api (only the migrate profile may set it true later, §12/D.5); no Flyway migration SQL authored in this phase.
- [ ] Config keys are declared (not implemented) consistent with Appendix D.2 wiring expectations — `SPRING_PROFILES_ACTIVE`, `app.org.timezone` (`ORG_TIMEZONE`, default `America/Chicago` with the documented fail-safe to `America/Chicago` + WARN), `VITE`-side untouched — but NO auth/CORS/SNS behavior is implemented (those are later-phase). App must start under `local` and `demo` profiles.
- [ ] Files: NEW `apps/wc-api/api/src/main/java/com/st6/wc/WcApiApplication.java`, NEW `apps/wc-api/api/src/main/resources/{application.yml,application-prod.yml,application-local.yml,application-demo.yml,application-flyway-migrate.yml}` (Appendix C.2)
- [ ] Cross-doc invariant: none (no model fields introduced)
- [ ] Tests — happy: `@SpringBootTest` context loads under `local` profile and `/actuator/health/readiness` returns 200/UP; edge: `flyway-migrate` profile resolves `spring.flyway.enabled=true` while base resolves `false`; error: an undefined/blank `ORG_TIMEZONE` falls back to `America/Chicago` + WARN (no crash) per D.6 fail-safe; integration: app boots under both `local` and `demo` profiles
- [ ] Requirements: REQ-O-015, REQ-O-016

### 0.5 — `wc-sync-worker` base Spring Boot app: separate deployable, actuator, no consume logic
- [ ] `:worker` produces a separate runnable Spring Boot app `WcSyncWorkerApplication.java` (distinct deployable per REQ-O-014/§2) depending on `:shared`, booting with NO SQS listener wired and NO Graph adapter behavior (deferred); exposes actuator `health/liveness` + `health/readiness` (§15).
- [ ] Worker profile resources exist per Appendix C.3: `application.yml`, `application-local.yml`, `application-demo.yml`, `application-prod.yml`; `spring.flyway.enabled=false` always (worker never migrates, §12/D.3); worker declares NO Auth0/SNS/CORS/timezone config (per D.3 note, it carries no user request).
- [ ] The in-process async dispatcher (`InProcessSyncDispatcher`) and SQS listener are NOT implemented here; only the deployable + health surface + profile skeleton, proving the worker is a separate image boundary.
- [ ] Files: NEW `apps/wc-api/worker/src/main/java/com/st6/wc/worker/WcSyncWorkerApplication.java`, NEW `apps/wc-api/worker/src/main/resources/{application.yml,application-local.yml,application-demo.yml,application-prod.yml}` (Appendix C.3)
- [ ] Cross-doc invariant: none
- [ ] Tests — happy: worker `@SpringBootTest` context loads under `local` and `/actuator/health/readiness` returns UP; edge: `spring.flyway.enabled` resolves `false` in every worker profile; error: worker boots even with no SQS/Graph env set (no crash, since no listener wired yet); integration: worker JAR builds as a separate artifact from the api JAR
- [ ] Requirements: REQ-O-014, REQ-O-016, REQ-O-015
...
### 1.1 — Enum types, common base entities & Clock/Org-time config
- [ ] Create all 16 enum types as Java enums whose constant names equal the Appendix B.1 / Appendix A wire values exactly: `PlanState{DRAFT,LOCKED,RECONCILING,RECONCILED}`, `RoleType{IC,MANAGER}`, `CommitmentKind{PLANNED,UNPLANNED}`, `Priority{P0,P1,P2}`, `WorkType{STRATEGIC,MAINTENANCE,BLOCKER,UNPLANNED}`, `Confidence{HIGH,MEDIUM,LOW}`, `AlignmentStatus{ALIGNED,NEEDS_REVIEW,MISALIGNED}`, `ReviewStatus{NOT_REVIEWED,REVIEWED_WITH_DISPUTES,REVIEWED}` (NO `OVERDUE` constant — derived only), `DisputeStatus{OPEN,IC_RESPONDED,RESOLVED}`, `ReconciliationOutcome{COMPLETED,PARTIALLY_COMPLETED,BLOCKED,CANCELED,CARRIED_FORWARD}`, `FlagType{NEEDS_REVISION,MISALIGNED}`, `CommentTargetType{PLAN,COMMITMENT}` (only two — not 4), `SyncRelatedType{WEEKLY_PLAN,MANAGER_REVIEW_WEEK}`, `EventKind{IC_PLANNING,IC_RECONCILIATION,MANAGER_REVIEW_BLOCK}`, `SyncStatus{PENDING_PUBLISH,QUEUED,SYNCING,SYNCED,FAILED,RETRY_REQUESTED}`, `RiskBadge{MISALIGNED,NEEDS_REVIEW,BLOCKED,CARRY_FORWARD,UNREVIEWED,OVERDUE_REVIEW}`.
- [ ] `AbstractAuditingEntity` (`@MappedSuperclass`) exposes created_by/created_at/updated_by/updated_at audit columns; `PersistableUuidEntity` provides the UUID PK + `@Version` optimistic-lock token base; Lombok `@Getter @Setter @Builder` (never `@Data`).
- [ ] `ClockConfig` exposes an injectable `java.time.Clock` bean (system default) for §3/§17 derived-state computation; `OrgTimeConfig` exposes org tz (`America/Chicago` default, env-overridable) and a Monday–Sunday week resolver.
- [ ] Files: NEW `apps/wc-api/shared/src/main/java/com/st6/wc/enums/{PlanState,RoleType,CommitmentKind,Priority,WorkType,Confidence,AlignmentStatus,ReviewStatus,DisputeStatus,ReconciliationOutcome,FlagType,CommentTargetType,SyncRelatedType,EventKind,SyncStatus,RiskBadge}.java`; NEW `apps/wc-api/shared/src/main/java/com/st6/wc/common/{AbstractAuditingEntity,PersistableUuidEntity}.java`; NEW `apps/wc-api/api/src/main/java/com/st6/wc/config/ClockConfig.java`; NEW `apps/wc-api/shared/src/main/java/com/st6/wc/common/OrgTimeConfig.java`.
- [ ] Cross-doc invariant: NEW (enum vocabulary for PlanState, ReviewStatus, DisputeStatus, ReconciliationOutcome, EventKind, SyncStatus, RiskBadge, CommentTargetType — all Appendix A enum-bearing models depend on these exact constant sets).
- [ ] Tests — happy: each enum’s `values()` equals the Appendix B.1 set in count and spelling; edge: `ReviewStatus` has no `OVERDUE` constant, `CommentTargetType` has exactly `PLAN`/`COMMITMENT`, `SyncRelatedType` has `MANAGER_REVIEW_WEEK` (not `MANAGER_REVIEW`); error: `valueOf` on a stale name (`OVERDUE`, `MANAGER_REVIEW`) throws; integration: Clock bean is overridable with a fixed `Clock` for deterministic tests.
- [ ] Requirements: REQ-D-010.

### 1.2 — Flyway V1: core schema DDL (12 tables, VARCHAR+CHECK, FKs, audit cols)
- [ ] `V1__core_schema.sql` creates all core tables with UUID PKs (REQ-D-009): `employee`, `manager_relationship`, `rally_cry`, `defining_objective`, `supporting_outcome`, `weekly_plan`, `weekly_commitment`, `manager_review`, `alignment_dispute`, `comment`, `outlook_calendar_sync_record`, `audit_event` — plus the `AbstractAuditingEntity` columns on every domain table.
- [ ] All lifecycle/status columns are `VARCHAR` with `CHECK` constraints (NOT native enums, REQ-D-010): `weekly_plan.state` CHECK in the 4 PlanState values; `weekly_commitment.commitment_kind`/`priority`/`work_type`/`confidence`/`alignment_status`/`reconciliation_outcome` CHECK in their Appendix A sets; `manager_review.status` CHECK in the 3 source statuses (NO `OVERDUE`); `alignment_dispute.status`/`flag_type` CHECK; `comment.target_type` CHECK in `('PLAN','COMMITMENT')` only; `outlook_calendar_sync_record.related_type` CHECK in `('WEEKLY_PLAN','MANAGER_REVIEW_WEEK')`, `event_kind` CHECK, `status` CHECK in the 6 SyncStatus values; `employee.role` CHECK in `('IC','MANAGER')`.
- [ ] Encode the four contract deltas vs DATA_MODEL.md (contract wins): `weekly_commitment.manager_alignment_note text null` is PRESENT (NEW); `weekly_commitment.progress_status` is ABSENT (dropped); `comment` keeps `parent_comment_id`/`path`/`depth` columns but `path` is nullable (flat MVP, `depth default 0`); `outlook_calendar_sync_record` has `week_start_date date null` (NEW) and uses `related_type` value `MANAGER_REVIEW_WEEK`.
- [ ] `@Version` integer column present on the five mutable lifecycle tables: `weekly_plan`, `weekly_commitment`, `manager_review`, `alignment_dispute`, `outlook_calendar_sync_record`.
- [ ] FKs wired per Appendix A: relationship→employee (mgr+report), DO→rally_cry, SO→DO, plan→employee, commitment→plan, commitment.supporting_outcome_id→SO (nullable), commitment.carry_forward_source_commitment_id→self (nullable), review→plan(unique)+employee, dispute→commitment+employee, comment.author→employee + nullable parent→comment, sync.owner→employee, audit.actor_employee_id→employee (NULLABLE for SYSTEM).
- [ ] Non-unique indexes from §4/DATA_MODEL: commitment `(weekly_plan_id, commitment_kind)`, `(supporting_outcome_id)`, `(carry_forward_source_commitment_id)`, `(alignment_status)`, plus the §4-added `(priority)` and `(work_type)` to back command-center filters; plan `(employee_id, state)`, `(week_start_date, state)`; review `(review_due_at)`; dispute `(commitment_id, status)`; sync `(owner_employee_id, event_kind)`; audit `(entity_type, entity_id)`, `(action, created_at)`.
- [ ] `weekly_plan` carries the `unique(employee_id, week_start_date)` constraint (REQ-D-002); `employee.email` unique; `manager_relationship` unique`(manager_employee_id, direct_report_employee_id)` + CHECK manager≠report; `manager_review.weekly_plan_id` unique.
- [ ] Files: NEW `apps/wc-api/shared/src/main/resources/db/migration/V1__core_schema.sql`.
- [ ] Cross-doc invariant: NEW (Employee, ManagerRelationship, RallyCry, DefiningObjective, SupportingOutcome, WeeklyPlan, WeeklyCommitment[incl. manager_alignment_note NEW, progress_status ABSENT], ManagerReview, AlignmentDispute, Comment, OutlookCalendarSyncRecord[incl. week_start_date NEW], AuditEvent — Appendix A models).
- [ ] Tests — happy: Flyway V1 migrates clean on a Testcontainers PG16; all 12 tables exist with expected columns; edge: inserting a row with an out-of-vocabulary status (e.g. `weekly_plan.state='ARCHIVED'`, `comment.target_type='MANAGER_REVIEW'`, `sync.related_type='MANAGER_REVIEW'`) is rejected by the CHECK constraint; `weekly_commitment` has no `progress_status` column and has a `manager_alignment_note` column; error: duplicate `(employee_id, week_start_date)` insert raises a unique-violation; FK insert with a non-existent `supporting_outcome_id` raises an FK violation; integration: a row with `actor_employee_id` NULL inserts into `audit_event` (SYSTEM actor path).
- [ ] Requirements: REQ-D-001, REQ-D-002, REQ-D-009, REQ-D-010, REQ-D-011.

### 1.3 — Flyway V2: partial unique indexes (single-active-manager, one-open-dispute, per-manager/week review-block)
- [ ] `V2__partial_unique_indexes.sql` creates the partial unique index on `manager_relationship (direct_report_employee_id) WHERE active = true` — enforces single active manager per direct report (§4/§6).
- [ ] Creates the partial unique index on `alignment_dispute (commitment_id) WHERE status IN ('OPEN','IC_RESPONDED')` — at most one unresolved dispute per commitment (§3/§4), carried verbatim from the contract.
- [ ] Creates the partial unique index on `outlook_calendar_sync_record (owner_employee_id, week_start_date) WHERE event_kind = 'MANAGER_REVIEW_BLOCK'` — one review-block event per manager/week grain (§4/§10), in addition to the existing full unique `(owner_employee_id, related_type, related_id, event_kind)` for plan-related kinds (created here or in V1 — keep both consistent).
- [ ] Files: NEW `apps/wc-api/shared/src/main/resources/db/migration/V2__partial_unique_indexes.sql`.
- [ ] Cross-doc invariant: extended (ManagerRelationship single-active-manager partial-unique, AlignmentDispute one-unresolved partial-unique, OutlookCalendarSyncRecord per-manager/week review-block partial-unique — Appendix A constraint clauses).
- [ ] Tests — happy: V2 migrates clean after V1; a second `active=true` relationship for a report whose first manager is `active=false` succeeds; error: a second `active=true` `manager_relationship` for the same direct_report raises a unique violation; opening a second `OPEN` (or `IC_RESPONDED`) dispute on the same `commitment_id` raises a unique violation while a `RESOLVED` dispute does NOT block a new one; a second `MANAGER_REVIEW_BLOCK` sync record for the same `(owner_employee_id, week_start_date)` raises a unique violation while `IC_PLANNING`/`IC_RECONCILIATION` records for the same owner/week do NOT; integration: the full unique `(owner,related_type,related_id,event_kind)` still blocks a duplicate `IC_PLANNING` for the same plan.
- [ ] Requirements: REQ-D-001, REQ-D-002.

### 1.4 — Flyway V3: synchronous projection tables (manager_plan_summary, manager_heatmap_cell)
- [ ] `V3__projection_tables.sql` creates `manager_plan_summary` (one row per manager/report/week) with `plan_state`, `review_status` (nullable), `review_due_at`, `is_review_overdue boolean default false`, and the seven count columns (planned/unplanned/misaligned/needs_review/blocked/carry_forward/unresolved_dispute), `updated_at`, and `unique(manager_employee_id, employee_id, week_start_date)` per §9/Appendix A.
- [ ] Creates `manager_heatmap_cell` (manager × report × week × Defining Objective) with `commitment_count` + the same seven count columns, `risk_badges text[] not null default '{}'` constrained to the enumerated badge vocabulary `{MISALIGNED,NEEDS_REVIEW,BLOCKED,CARRY_FORWARD,UNREVIEWED,OVERDUE_REVIEW}`, `updated_at`, and `unique(manager_employee_id, employee_id, week_start_date, defining_objective_id)`.
- [ ] Supporting indexes per DATA_MODEL: summary `(manager_employee_id, week_start_date, plan_state)`, `(manager_employee_id, review_status)`; heatmap `(manager_employee_id, week_start_date)`, `(manager_employee_id, defining_objective_id)`; FKs to employee/weekly_plan/defining_objective.
- [ ] Files: NEW `apps/wc-api/shared/src/main/resources/db/migration/V3__projection_tables.sql`.
- [ ] Cross-doc invariant: NEW (ManagerPlanSummary, ManagerHeatmapCell — Appendix A projection models; `risk_badges text[]` enumerated vocabulary).
- [ ] Tests — happy: V3 migrates clean after V1/V2; both projection tables exist with all count columns and `risk_badges` as a Postgres `text[]`; a row with `risk_badges='{MISALIGNED,OVERDUE_REVIEW}'` round-trips; edge: empty `risk_badges` defaults to `'{}'`; error: duplicate `(manager_employee_id, employee_id, week_start_date)` summary insert raises a unique violation; duplicate `(manager,employee,week,defining_objective)` heatmap insert raises a unique violation; integration: a `risk_badges` array element outside the enumerated vocabulary is rejected (CHECK over array membership, or asserted at the entity converter boundary if DB-level array CHECK is not used).
- [ ] Requirements: REQ-D-001, REQ-D-009, REQ-D-010.

### 1.5 — JPA entities mirroring Appendix A (mapped to V1–V3 tables)
- [ ] Author one JPA entity per table, each extending `PersistableUuidEntity`/`AbstractAuditingEntity`, with field names matching Appendix A and `@Enumerated(EnumType.STRING)` columns mapped to the VARCHAR+CHECK columns (entity enum ↔ DB string): `Employee`, `ManagerRelationship`, `RallyCry`, `DefiningObjective`, `SupportingOutcome`, `WeeklyPlan`, `WeeklyCommitment`, `ManagerReview`, `AlignmentDispute`, `Comment`, `OutlookCalendarSyncRecord`, `AuditEvent`, `ManagerPlanSummary`, `ManagerHeatmapCell`.
- [ ] `@Version` field present on `WeeklyPlan`, `WeeklyCommitment`, `ManagerReview`, `AlignmentDispute`, `OutlookCalendarSyncRecord` (optimistic locking, §4); the five entities map their version column to the V1 DDL.
- [ ] `WeeklyCommitment` includes `managerAlignmentNote` (NEW, manager-owned, post-lock-mutable, nullable), `alignmentStatus` (AlignmentStatus), `reconciliationOutcome`, `carryForwardSourceCommitmentId` self-reference, and has NO `progressStatus` field. `OutlookCalendarSyncRecord` includes `weekStartDate` (NEW) and `relatedType` mapping `MANAGER_REVIEW_WEEK`. `Comment` includes `parentCommentId`/`path`/`depth` fields (flat MVP defaults: parent NULL, depth 0) and `targetType` over the 2-value enum. `AuditEvent.actorEmployeeId` nullable.
- [ ] `ManagerHeatmapCell.riskBadges` maps the `text[]` column to a typed list of `RiskBadge` (JPA array/converter), default empty.
...
- [ ] Cross-doc invariant: NEW (all 14 Appendix A models — every field name and enum binding must match Appendix A; `manager_alignment_note` NEW, `week_start_date` NEW, `progress_status` ABSENT, comment `target_type` {PLAN,COMMITMENT}).
- [ ] Tests — happy: each entity persists and reloads via its repository against the Flyway-migrated Testcontainers schema; enum fields round-trip as their string value; edge: `WeeklyCommitment` with NULL `managerAlignmentNote`/`supportingOutcomeId` persists; `Comment` persists with `parentCommentId` NULL and `depth=0`; `ManagerHeatmapCell` persists with a multi-element `riskBadges` list; error: persisting an entity whose enum maps to an out-of-CHECK string (forced via native query) is rejected by the DB; integration: a `WeeklyCommitment` self-referencing `carryForwardSourceCommitmentId` to another persisted commitment loads its source link.
- [ ] Requirements: REQ-D-001, REQ-D-006, REQ-D-009, REQ-D-010, REQ-D-011, REQ-D-014.

### 1.6 — Spring Data repositories + Testcontainers constraint-proof tests
- [ ] One Spring Data JPA repository per entity under each `<domain>/repo/` package; repositories include the partial-unique-aware finder queries needed by later phases (e.g. active-manager-for-report lookup, unresolved-dispute-for-commitment lookup, review-block-for-manager/week lookup) without yet implementing business logic.
- [ ] A shared Testcontainers PostgreSQL 16.x base test (no H2, per §13/§17) that runs the V1–V3 Flyway migrations against a real PG container; all repository tests extend it.
- [ ] Constraint-proof integration tests assert each locked invariant fires at the DB level: unique `(employee_id, week_start_date)` on plan; single-active-manager partial unique; one-unresolved-dispute partial unique; per-manager/week review-block partial unique; CHECK rejection on out-of-vocabulary status values; optimistic-lock `@Version` increments on update (and a stale-version update raises `OptimisticLockException`).
- [ ] Files: NEW `apps/wc-api/shared/src/main/java/com/st6/wc/{employee,relationship,rcdo,plan,commitment,review,dispute,comment,sync,projection,audit}/repo/<Entity>Repository.java`; NEW `apps/wc-api/api/src/test/java/com/st6/wc/` Testcontainers base + per-table constraint tests.
- [ ] Cross-doc invariant: extended (repositories and constraint tests are the executable proof of the Appendix A constraint clauses; no new model surface).
- [ ] Tests — happy: every repository saves+finds against the migrated schema; edge: finder query for "active manager of report" returns exactly one row when one active + one inactive relationship exist; error: each partial-unique and CHECK violation raises the expected `DataIntegrityViolationException`; stale `@Version` update raises optimistic-lock conflict; integration: V1→V2→V3 apply in order on a fresh container and a re-run of the suite is repeatable (deterministic schema).
- [ ] Requirements: REQ-D-001, REQ-D-002, REQ-D-009, REQ-D-010, REQ-D-011.

### Acceptance criteria (1)
- [ ] All 1.X task checkboxes ticked.
- [ ] Flyway V1–V3 migrate clean and in order against a Testcontainers PostgreSQL 16.x container (no H2 anywhere).
- [ ] Every enum in `com.st6.wc.enums` matches Appendix B.1 / Appendix A constant-for-constant (incl. `ReviewStatus` having NO stored `OVERDUE`, `CommentTargetType` having exactly `{PLAN,COMMITMENT}`, `SyncRelatedType` using `MANAGER_REVIEW_WEEK`).
- [ ] The four contract deltas are encoded and tested: `manager_alignment_note` PRESENT, `progress_status` ABSENT, `outlook_calendar_sync_record.week_start_date` PRESENT, comments flat-but-nestable.
- [ ] All three partial unique indexes (single-active-manager, one-open-dispute, per-manager/week review-block) provably fire in DB-level tests.
- [ ] `@Version` optimistic locking is present and proven on the five mutable lifecycle entities.
- [ ] All 14 Appendix A models exist as JPA entities with matching field names + repositories, tagged NEW in the cross-doc-invariants table.
...
### 2.1 — Auth0 JWT decoder: RS256-only, issuer + custom audience mandatory
- [ ] `JwtConfig` exposes a `JwtDecoder` built from `JwtValidators.createDefaultWithIssuer(issuerUri)` (issuer mandatory, `exp`/`nbf` enforced with bounded clock skew).
- [ ] A custom `AudienceValidator` (implements `OAuth2TokenValidator<Jwt>`) checks the `aud` claim contains the configured `auth0.audience`; combined with the default+issuer validators via `DelegatingOAuth2TokenValidator` (Auth0 does not validate audience by default — §6).
- [ ] Decoder rejects `alg=none` and any symmetric (HS*) algorithm; only `RS256` is accepted; tokens missing `iss` or `aud` are rejected.
- [ ] Reads `spring.security.oauth2.resourceserver.jwt.issuer-uri`, optional `jwk-set-uri`, and `auth0.audience` from Spring properties (Appendix D.2); both issuer and audience are required in real mode (D.6) and absence fails fast with a clear startup error (not a silent insecure default).
- [ ] Real-mode auth path is active only when not in demo mode; demo mode leaves the validated JWT path inactive (D.6).
- [ ] Files: NEW `apps/wc-api/api/src/main/java/com/st6/wc/config/JwtConfig.java`, NEW `apps/wc-api/api/src/main/java/com/st6/wc/config/AudienceValidator.java`; extended `apps/wc-api/api/src/main/resources/application.yml` + `application-prod.yml` (jwt/audience keys).
- [ ] Cross-doc invariant: none (config/validator behavior; no Appendix-A model field change).
- [ ] Tests — happy: valid RS256 JWT with correct issuer + audience passes validation. edge: token with extra audiences containing the required one still passes; expired (`exp` past) within skew vs beyond skew. error: `alg=none` rejected; HS256 (symmetric) rejected; wrong-issuer rejected; missing/wrong `aud` rejected; missing `aud` claim rejected. integration: `OAuth2TokenValidatorResult` carries a failure for the wrong-audience case (asserted by the §17 "wrong-audience JWT rejected" security test).
...
- [ ] Org-tz fail-safe: if `app.org.timezone`/`ORG_TIMEZONE` is unset/blank/unparseable as a `ZoneId`, fall back to `America/Chicago` and log WARN — never start with an undefined zone, never silently use UTC (Appendix D.4).
...
- [ ] **HITL deploy runbook (`docs/runbooks/`) — author alongside 12.11 / at round close.** Accumulated HITL prereqs + steps the deploy needs (we author+validate only): (1) `terraform init -backend-config=bucket/key/region/dynamodb_table/encrypt` against the HITL-created S3 state bucket [12.1]; (2) the Route53 hosted zone for `ROOT_DOMAIN` must pre-exist (domain registered/delegated) [12.7a]; (3) the GitHub `production` Environment + required reviewers must exist for the env-scoped OIDC token to issue [12.7c]; (4) verify the constructed `ci_boundary` ARN matches the real policy ARN at apply (else the self-referencing Denies misbind) [12.7c]; (5) the full apply→migration-Job→deploy→S3-sync+CF-invalidation→smoke chain [12.11/§13]. _(origin: 2026-06-02 12.1/12.7a/12.7c)_
...
- **HEAD:** `dfa1448` (last slice) / `b5a3882` (session doc) + this round's `/orchestrate-end` terminal commit. Branch `infra-track`.
- **Done:** 12.1–12.7 (a/b/c) + 12.10 ✅. **Next: 12.8** — brief `docs/briefs/010-12.8-k8s-sa-spc-svc-ingress-externaldns.md` is authored + ready.
- **Rulings to honor (docs/decisions/001):** D1 full-apply hardened CI · D2 external-dns owns `api.wc` (C.7 updated) · D3 guardrail permissions boundary (`Allow *` + Denies) on EVERY TF-created role.
- **Hard contracts for 12.8:** 12.7b↔12.8 SA-identity = namespace `wc`; SA names `wc-api`/`wc-worker`/`wc-cronjob`/`wc-migration`. external-dns role MUST carry `permissions_boundary = aws_iam_policy.ci_boundary.arn`. Per-SA SecretProviderClass exposes ONLY that SA's permitted secrets (worker→graph-only; cron/migration→db-only; api→db/auth0/graph). TF-output→manifest values via `${PLACEHOLDER}` + CI `envsubst`.
- **Then:** 12.9 (flyway ONLY on the migration Job) → 12.11 (full §13 pipeline: gates→build→apply→migrate→deploy→S3 sync+CF invalidation→smoke; env-scoped OIDC + `production` Environment gate; `actionlint`-clean).
- **Toolchain:** install `kubeconform` (12.8) + `actionlint` (12.11) at first use. Present: terraform 1.15.5 / tflint / kubectl / aws.
