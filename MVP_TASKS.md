...
- **(Frontend) Expose layout tokens into the Tailwind theme when layout work lands.** ST.1 mapped color/spacing/radii/shadow/motion tokens into `tailwind.config`, but `--reading-col`/`--content-max` aren't yet in the theme (`maxWidth`); `App.tsx` uses one arbitrary-value escape hatch `max-w-[var(--reading-col)]`. Fold into the first Phase 9 layout/shell slice — **retargeted to 9.7** (9.2 shipped view-states/badges, not layout; 9.4 is the route tree, also not layout). _(origin: 2026-06-02 ST.1; retriaged 2026-06-02 frontend resume)_
- **(Frontend, 9.5) Persona-aware `/` default + real `isManager` source.** 9.4 landed the lazy route tree with two seams pending real data: (a) the `/` route reuses the `App` shell as an interim landing — wire the **persona-aware default redirect** (manager → command center, IC → weekly-commit) once the role is known; (b) swap `routes/isManager.ts`'s `useIsManager()` (today a context defaulting to IC/false) to read `meApi`'s `MeDto.isManager` (B.3). The `AppRoutes` gating + manager-route registration consume the hook unchanged. Also: **re-run the auth0 build-grep** (LESSONS §6 fail-closed half) at the first feature slice that pulls `baseApi` into the remote build closure — likely 9.5 — since 9.4's placeholder pages don't import it (the static guard + source scan suffice for 9.4). _(origin: 2026-06-02 9.4)_
- **(Frontend, phases) Replace the 9.4 `routes/pages/*` placeholders with real views.** 9.4 registered four labeled placeholder route modules so each lazy chunk resolves; replace them with the real surfaces as their phases land: `WeeklyCommitPage` → 9.7, `PlanHistoryPage` → 9.8, `CommandCenterPage` → 9.9, `HeatmapPage` → 9.10. (Inline-target candidate at `/orchestrate-end` — these map to existing phase tasks.) _(origin: 2026-06-02 9.4)_
